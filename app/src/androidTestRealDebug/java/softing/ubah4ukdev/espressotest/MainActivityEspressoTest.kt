...
import softing.ubah4ukdev.espressotest.view.search.MainActivity
...
class MainActivityEspressoTest {
    ...
    @Test
    fun activitySearch_SendRequest_ReturnsTrue() {
        onView(withId(R.id.searchEditText)).perform(typeText(QUERY))
        onView(withId(R.id.searchEditText)).perform(pressImeActionButton())
        onView(isRoot()).perform(delay())
        scenario.onActivity {
            val returnCount = it.recyclerView.adapter?.itemCount ?: 0
            assertTrue(returnCount > 0)
        }
    }

    companion object {
        private const val NUMBER_RESULT = "Number of results: 2657"
